...
export const useRooms = (category?: string) => {
  ...
  const updateStatus = useMutation({
    mutationFn: async ({ roomId, statusId, observation, actionType, employeeId, statusDate, stayId }: { roomId: string, statusId: string, observation?: string, actionType: string, employeeId?: string, statusDate?: string, stayId?: string }) => {
      const { data: currentRoom } = await supabase.from('rooms').select('status_id, status_date').eq('id', roomId).single();
