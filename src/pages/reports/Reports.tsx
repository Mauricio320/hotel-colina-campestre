...
const Reports: React.FC<ReportsProps> = ({ userRole }) => {
  ...
  const handleMonthlyReport = async () => {
    ...
    const { data, error } = await supabase
      .from("stays")
      .select(
        "order_number, check_in_date, total_price, paid_amount, status, guest:guests(first_name, last_name), room:rooms(room_number)",
      )
    ...
    if (data) {
      ...
      exportToCSV(
        formatted,
        `reporte_pagos_${new Date().toISOString().slice(0, 7)}.csv`,
      );
    }
    setLoading(false);
  };
  ...
  const handlePaymentsReport = async () => {
    ...
    try {
      ...
      if (paymentsData) {
        ...
        exportToCSV(
          formatted,
          `reporte_pagos_detallado_${new Date().toISOString().slice(0, 7)}.csv`,
        );
      }
    } catch (error) {
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        >
          <p className="text-gray-600 mb-6">
            Detalle consolidado de todos los pagos, abonos y deudas por estadía
            a la fecha actual.
          </p>
          <Button
        ...
        >
          <p className="text-gray-600 mb-6">
            Historial completo de pagos y abonos con método, tipo y empleado que
            registró.
          </p>
          <Button
