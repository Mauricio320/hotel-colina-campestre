...
import { PaymentType, CreatePaymentDto } from "@/types";
...
export const useStays = () => {
  ...
  const registerPayment = useMutation({
    mutationFn: async ({
      ...
      // Determine payment type and create payment record
      ...
      // Update stay paid_amount and status
      ...
      // REMOVED: Automatic room status change on full payment
      // Room status will now be changed manually via check-in modal
      // if (isFullyPaid && stay.check_in_date <= todayStr && roomId) {
  ...
  const createStayWithPayment = useMutation({
    mutationFn: async (data: {
      ...
      try {
        ...
        // Update room status based on payment type and dates
        const todayStr = new Date().toLocaleDateString("sv-SE");
        const { data: availableStatus } = await supabase
          .from("room_statuses")
          .select("id")
        ...
        if (statusData) {
          ...
          await supabase.from("room_history").insert({
            room_id: data.stayData.room_id,
            stay_id: stay.id,
            previous_status_id:
              availableStatus?.id || currentRoomStatus?.status_id,
            new_status_id: statusData.id,
            employee_id: data.paymentData.employee_id,
            action_type:
